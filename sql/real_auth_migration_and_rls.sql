...
revoke all on function public.accept_order(bigint) from public,anon;
...
begin
  ...
  if old.status='pending' and old.accepted_by_tech is null and old.tech_id is null then
    ...
    if old.user_id is distinct from new.user_id or old.name is distinct from new.name or old.phone is distinct from new.phone or old.service is distinct from new.service or old.date is distinct from new.date or old.time is distinct from new.time or old.governorate is distinct from new.governorate or old.notes is distinct from new.notes or old.photo is distinct from new.photo or old.invoice_id is distinct from new.invoice_id or old.invoice_number is distinct from new.invoice_number or old.invoice_amount is distinct from new.invoice_amount or old.payment_method is distinct from new.payment_method or old.created_at is distinct from new.created_at then
      raise exception 'الفني لا يستطيع تعديل بيانات الطلب أو العميل أثناء القبول';
    end if;
    return new;
  end if;
  if old.tech_id is distinct from v_tech_id or old.accepted_by_tech is distinct from v_tech_id then raise exception 'لا يمكنك تعديل طلب فني آخر'; end if;
  if old.user_id is distinct from new.user_id or old.name is distinct from new.name or old.phone is distinct from new.phone or old.service is distinct from new.service or old.date is distinct from new.date or old.time is distinct from new.time or old.governorate is distinct from new.governorate or old.notes is distinct from new.notes or old.photo is distinct from new.photo or old.tech_id is distinct from new.tech_id or old.accepted_by_tech is distinct from new.accepted_by_tech or old.tech_name is distinct from new.tech_name or old.tech_phone is distinct from new.tech_phone or old.tech_accepted_at is distinct from new.tech_accepted_at or old.created_at is distinct from new.created_at then
