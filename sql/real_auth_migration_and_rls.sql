...
create index if not exists orders_pending_assignment_idx on public.orders (status, accepted_by_tech) where accepted_by_tech is null;
...
returns bigint language sql stable security definer set search_path = '' as $$
  select u.id from public.users as u where u.auth_uid = (select auth.uid()) limit 1;
$$;

create or replace function private.my_role()
returns text language sql stable security definer set search_path = '' as $$
  select coalesce(u.role, 'user') from public.users as u where u.auth_uid = (select auth.uid()) limit 1;
$$;
...
create policy "users_select_isolated" on public.users for select to authenticated using (
  (select private.my_role()) = 'admin'
  or auth_uid = (select auth.uid())
  or id in (select o.user_id from public.orders as o where o.tech_id = (select private.my_tech_id()))
);

create policy "users_insert_self" on public.users for insert to authenticated with check (
  auth_uid = (select auth.uid()) and coalesce(role, 'user') = 'user'
);

create policy "users_update_own" on public.users for update to authenticated
using ((select private.my_role()) = 'admin' or auth_uid = (select auth.uid()))
with check (
  (select private.my_role()) = 'admin'
  or (auth_uid = (select auth.uid()) and coalesce(role, 'user') = 'user')
);
...
create policy "orders_select_isolated" on public.orders for select to authenticated using (
  (select private.my_role()) = 'admin'
  or user_id = (select private.my_user_id())
  or tech_id = (select private.my_tech_id())
  or (status = 'pending' and accepted_by_tech is null and (select private.my_tech_id()) is not null)
);

create policy "orders_insert_own" on public.orders for insert to authenticated with check (
  (select private.my_role()) = 'admin' or user_id = (select private.my_user_id())
);

create policy "orders_update_admin_or_owner" on public.orders for update to authenticated
...
-- قبول الطلب يبقي الحالة الداخلية = accepted، بينما الواجهة تعرض النص العربي.
-- هذا يمنع كسر شريط التتبع الذي يعتمد على الحالات الداخلية.
...
create policy "technicians_select_authenticated" on public.technicians for select to authenticated using (
  (select private.my_role()) = 'admin' or id = (select private.my_tech_id())
);

create policy "technicians_update_admin_only" on public.technicians for update to authenticated
...
create policy "invoices_select_isolated" on public.invoices for select to authenticated using (
  (select private.my_role()) = 'admin'
  or customer_id = (select private.my_user_id())
  or tech_id = (select private.my_tech_id())
);

create policy "invoices_insert_admin_or_own_tech" on public.invoices for insert to authenticated with check (
  (select private.my_role()) = 'admin' or tech_id = (select private.my_tech_id())
);

create policy "invoices_update_admin_only" on public.invoices for update to authenticated
using ((select private.my_role()) = 'admin') with check ((select private.my_role()) = 'admin');
